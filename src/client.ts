...
export class GrpcClient {
    ...
    public getBip44HdAddress(): Promise<any> { return null}
    ...
    private async _checkNetworkIntegrity(callback: (result: boolean) => any): Promise<boolean> {
        if (typeof this._HAS_NETWORK_INTEGRITY !== "boolean") {
            ...
            try { 
                res = await this.getBlockInfo({ hash: "000000000000000033bd7e29c7d54dc0aeee73b45d0ba7cdf7785e47c190cb72", reversedHashOrder: true }); 
            } catch (e) {
                console.log("🚀 ~ GrpcClient ~ _checkNetworkIntegrity ~ e:", e)
            }
